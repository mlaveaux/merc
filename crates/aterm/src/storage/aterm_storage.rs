...
// never be the sentinel `usize::MAX` and to always be fully initialized. `#[repr(C)]` guarantees
// `symbol: SymbolRef<'static>` occupies those bytes (pinned by the `offset_of!` assertion in `mod
// tests` below), which are `symbol`'s `StablePointer<SharedSymbol>` -- a real, always-initialized
// address from `SymbolPool`'s allocator, never `usize::MAX`.
...
// SAFETY: Same reasoning as `SharedTermFixed<N>` above: `#[repr(C)]` (pinned by the `offset_of!`
// assertion in `mod tests` below) keeps `symbol` first. `annotation: usize` is not covered by
// `BlockAllocatorSafe`, which only constrains that leading `symbol` slot.
