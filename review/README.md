...
| Medium | Lambda-body widening: a lambda body needing sort widening (e.g. a `{ .. }` literal's `FSet`/`FBag` widening to `Set`/`Bag`, or a primitive needing to widen to a wider declared range) has no outer constraint to widen against and fails to type-check | `crates/typecheck/src/inference/inference.rs`, `crates/typecheck/src/lowering/mcrl2_lowering.rs` | **FIXED** (body widens via a fresh `Sub`-constrained variable; `lower_lambda` materializes the coercion at the body, since a function *sort* itself has no term-level coercion), verified against the mcrl2 oracle-conformance suite (see typecheck-lambda-and-join-widening-bugs.md) |
