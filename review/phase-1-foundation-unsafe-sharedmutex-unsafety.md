...
## Finding #1 outcome: FIXED

`RecursiveLockWriteGuard`'s `DerefMut` impl (the source of the aliasing — a bare `&mut T` has no `Drop` hook, so nothing could tell `read_recursive()` it was still live) is replaced by a scoped `pub fn with_mut<R>(&mut self, f: impl FnOnce(&mut T) -> R) -> R`. A new `mutating: Cell<bool>` field on `RecursiveLock` is set for the duration of `f` (reset on drop, panic-safe via an RAII guard) and `read_recursive()` now asserts it is clear before proceeding — closing both the original repro shape (stash `&mut *write` in a `let`, call `read_recursive()` afterwards — no longer expressible at all, since `DerefMut` is gone) and the reentrant shape (call `read_recursive()` from *inside* the closure passed to `with_mut` — now a deterministic panic instead of a silent alias).

- `test_deref_mut_alias_survives_nested_read_recursive` (the original UB regression test) is replaced by two tests: `test_read_recursive_during_with_mut_panics` (`#[should_panic]`, proves the reentrant case is now rejected) and `test_read_recursive_after_with_mut_succeeds` (proves the legitimate non-overlapping case still works). Both pass under plain `cargo test -p merc_sharedmutex --lib recursive_lock` (9/9 tests green).
- All three call sites in the crate's own test module using the old `*write`/`*guard` `DerefMut` syntax (`test_read_recursive_inside_write`, the loom test) updated to `write.with_mut(|v| ...)`.
- `cargo check -p merc_sharedmutex --tests`, `cargo clippy -p merc_sharedmutex --all-targets`, and `cargo +nightly fmt -p merc_sharedmutex -- --check` all clean.
- Miri confirmation of the fixed code (`cargo +nightly miri test -p merc_sharedmutex --lib recursive_lock`) was in progress at the time of this note — see the session for its result; not fixing that in the report text after the fact.
- **Callers outside this crate**: `crates/aterm` uses `RecursiveLockWriteGuard` in `crates/aterm/src/storage/thread_aterm_pool.rs` (only via method calls through the old auto-deref, never storing a raw `&mut T` across statements — not itself a second instance of the bug, just a caller needing a mechanical update to the new API). That update was handed to the agent already reviewing `crates/aterm` to avoid a concurrent-edit collision on the same files; verify it landed before treating the workspace as fully green.
